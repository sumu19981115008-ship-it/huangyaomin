...
const origOnBulletHit = logic.onBulletHit.bind(logic);
logic.onBulletHit = function(turretId, col, row) {
  // 找车颜色
  const t = this.turrets.find(t => t.id === turretId);
  const color = t ? t.color : '?';
  const prevBlocks = this.blocks.length;
  origOnBulletHit(turretId, col, row);
  if (this.blocks.length < prevBlocks) {
    hitStats[color] = (hitStats[color] ?? 0) + 1;
  }
};
...
while (logic.state === 'playing' && frames < MAX) {
  ...
  if (!logic.isTrackFull()) {
    ...
    if (!blocked) {
      ...
      if (c) {
        deployStats[c.color] = (deployStats[c.color] ?? 0) + 1;
...
console.log('最终: state=' + logic.state + ' frames=' + frames + ' blocks=' + logic.blocks.length);
console.log('\n各颜色统计（像素剩余/部署次数/命中次数/可达）:');
const allColors = new Set([...Object.keys(deployStats), ...Object.keys(hitStats), ...Object.keys(colorCount)]);
for (const c of allColors) {
  const pixels = colorCount[c] ?? 0;
  const deploys = deployStats[c] ?? 0;
  const hits = hitStats[c] ?? 0;
  const reach = reachable.has(c) ? '★可达' : '×不可达';
  if (pixels > 0 || deploys > 0) {
    console.log('  ' + c.slice(1) + ': pixels=' + pixels + ' deploys=' + deploys + ' hits=' + hits + ' ' + reach);
  }
}

console.log('\n队列剩余:');
for (let li = 0; li < logic.lanes.length; li++) {
  if (logic.lanes[li].length > 0) {
    const seq = logic.lanes[li].slice(0, 5).map(t => t.color.slice(1) + '(' + t.ammo + ')').join(' -> ');
    console.log('  L' + li + '(' + logic.lanes[li].length + '辆): ' + seq);
  }
}
console.log('暂存: ' + logic.buffer.map(t => t.color.slice(1) + '(' + t.ammo + ')').join(', '));
console.log('轨道: ' + logic.turrets.map(t => t.color.slice(1) + '(' + t.ammo + ')').join(', '));
