/**
 * 停车场降权参数网格搜索
 * 目标：同时通过 B24（新解锁）且不新增 A216/218/231 卡关
 */
...
// 目标关卡：b24必须通，a216/218/231不能新增卡关
const targets = [
  { dir: 'levels_b2', n: 24, mustWin: true },
  { dir: 'levels_a2', n: 216, mustWin: true },
  { dir: 'levels_a2', n: 218, mustWin: true },
  { dir: 'levels_a2', n: 231, mustWin: true },
  // 老卡关，允许卡
  { dir: 'levels_b2', n: 62 },
  { dir: 'levels_b2', n: 40, mustWin: true },
  { dir: 'levels_b2', n: 41, mustWin: true },
  { dir: 'levels_a2', n: 137, mustWin: true },
  { dir: 'levels_a2', n: 170, mustWin: true },
];
...
const baseWs    = [0.05, 0.08, 0.1, 0.12, 0.15, 0.2];
...
for (const baseW of baseWs) {
  for (const ammoScale of ammoScales) {
    for (const minFree of minFrees) {
      let wins = 0, mustFails = [];
      for (const { dir, n, mustWin } of targets) {
        const r = simulate(allData[dir + '_' + n], baseW, ammoScale, minFree);
        if (r === 'win') wins++;
        else if (mustWin) mustFails.push(dir.slice(-2) + n);
      }
      const perfect = mustFails.length === 0;
      if (perfect && wins > bestScore) { bestScore = wins; best = { baseW, ammoScale, minFree }; }
      if (perfect) console.log(`✓ baseW=${baseW} ammoScale=${ammoScale} minFree=${minFree} -> wins=${wins}/${targets.length}`);
    }
  }
}
console.log('\n最优参数:', best, '得分:', bestScore + '/' + targets.length);
