...
> 每次新开发前阅读本文档。最后更新：2026-04-23（C 组 + 编辑器试玩 + 关卡自动生成器）
...
- 余数 r = count % 10
- r == 0：不动
- r ≤ 5：删掉该色 BFS 最浅（最外层）的 r 个像素（向下取整，视觉损失最小）
- r > 5：在该色像素的相邻空位补 (10−r) 个像素（向上取整）；若棋盘全满则 fallback 删 r 个

函数修改的是 `pixels` 列表本身，`generate_queue_group()` 返回 `(lanes, pixels)` 供 `main()` 同步使用。
