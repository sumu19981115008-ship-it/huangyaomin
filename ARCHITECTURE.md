...
> 每次新开发前阅读本文档。最后更新：2026-04-23（AutoBot 策略升级：递推可达性 + 弹药匹配评分）
...
2. **递推可达性**（`_computeReachable`）：从四个方向向内扫描，遇到"有候选车的颜色"就穿透继续，遇到无候选车的颜色才停止。效果：阻挡方块本身可被消除 → 被遮挡方块也视为可达
3. **弹药匹配评分**：同色所有候选车弹药加总，`matchScore = 1 / (1 + |弹药总和 - 该色方块数|)`，差值越小得分越高
4. **空转过滤**：`idleLastLap=true` 且颜色在递推后仍不可达的车跳过，等路径被打开
5. **排序**：score 降序（不可达颜色得分 ×0.1 惩罚）> buffer 优先于队列
